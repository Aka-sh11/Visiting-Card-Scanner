...
def upload_file():
    global filename
    global start, last
    filename = filedialog.askopenfilename(initialdir='/Desktop', title='Select a card image',
    ...
    if filename.endswith('.JPG') or filename.endswith('.JPEG') or filename.endswith('.jpg') or filename.endswith(
            '.jpeg') or filename.endswith('.PNG') or filename.endswith('.png'):
...
def convert():
    try:
        ...
        tmsg.showwarning(
            title='Alert!', message='Please provide proper formatted image')
        return
    f_name = 'scanned' + filename[start + 1:last] + '.txt'
...
m1.add_command(label='Scan/Upload Visiting or Business cards and get all the text of cards', font=('Times', 13))
root.config(menu=mainmenu)
mainmenu.add_cascade(label='Aim', menu=m1)

m2 = Menu(mainmenu, tearoff=0)
m2.add_command(label='|| Passionate BackEnd Developer ||', font=('Times', 13))
m2.add_command(label='|| Coding Enthusiast ||', font=('Times', 13))
root.config(menu=mainmenu)
mainmenu.add_cascade(label='About us', menu=m2)

m3 = Menu(mainmenu, tearoff=0)
m3.add_command(label='E-mail: akasharma.py@gmail.com', font=('Times', 13))
m3.add_separator()
m3.add_command(label='Instagram: akasharma._', font=('Times', 13))
m3.add_separator()
m3.add_command(label='Github: https://www.github.com/Aka-sh11', font=('Times', 13))
root.config(menu=mainmenu)
...
Label(text='©copyright 2021', bg='#433E3B', fg='white', font=('Times', 10)).pack(side='bottom', fill='x')
Label(text='Developer: AKASH', bg='#433E3B', fg='white', font=('Times', 10, ' italic')).pack(side='bottom', fill='x')
